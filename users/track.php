...
$oid=$_GET['oid'];
$select="SELECT * FROM `orders` WHERE `id`=$oid AND `user_id`=$user_id";
   ...
   <section class="cart_section">
    ...
    <main>
       <section class="section3">
          ...
          if($status == 'delivered' && !empty($delivered)){
             $height="100%";
             $ball5_color="#4caf50";
             $ball4_color="#4caf50";
             $completed="100%";
          ...
          if(empty($delivered)){
           ...
           if(str_contains($fetch['delivery_option'],"self")){
               $out="Ready for Pickup";
               $height="75%";
              $completed='75%';
              $out_for_delivery=$fetch['date'];
              $ball4_color="#4caf50";
           }
            else{
           ...
           <section class="track_section">
               <section class="track_group">
                   <strong class="desc">
                   Track Order
               </strong>
               <div class="progress_parent">
                   <div style="width:<?php echo $height; ?>" class="progress_child">
                      <?php echo $completed; ?> complete
                   </div>
               </div>
               
               <section class="bar_section">
                  <section class="bar_line">
                      <div class="bar_child" style="height:<?php echo $height; ?>">
                          
                      </div>
                      <div style="top:0%" class="ball">
                        <span class="ball_det">
                            <strong style="white-space:nowrap">Order Placed</strong>
                            <span style="font-size:0.7rem;color:#708090;white-space:nowrap">Date:<?php echo $date; ?></span>
                        </span>  
                      </div>
                      
                       <div style="top:25%" class="ball">
                           <span class="ball_det">
                            <strong style="white-space:nowrap">Order Confirmed</strong>
                            <span style="font-size:0.7rem;color:#708090;white-space:nowrap">Date:<?php echo $date; ?></span>
                        </span>  
                      </div>
                       <div style="top:50%" class="ball">
                           <span class="ball_det">
                            <strong style="white-space:nowrap">Order Processing</strong>
                            <span style="font-size:0.7rem;color:#708090;white-space:nowrap">Date:<?php echo $date; ?></span>
                        </span>  
                      </div>
                       <div style="top:75%;background:<?php echo $ball4_color; ?>;" class="ball">
                           <span class="ball_det">
                            <strong style="white-space:nowrap"><?php echo $out; ?></strong>
                            <span style="font-size:0.7rem;color:#708090;white-space:nowrap">Date:<?php echo $out_for_delivery; ?></span>
                        </span>  
                      </div>
                       <div style="top:100%;background:<?php echo $ball5_color; ?>" class="ball">
                           <span class="ball_det">
                            <strong style="white-space:nowrap;">Delivered</strong>
                            <span style="font-size:0.7rem;color:#708090;white-space:nowrap">Date:<?php echo $delivered; ?></span>
                        </span>  
                      </div>
                  </section> 
               </section>
               ...
               <section class="track_group">
                   ...
                   <div class="order_details">
                       ...
                       <ul class="order_ul">
                           <li>
                               Order ID: <?php echo $fetch['uniqid']; ?>
                           </li>
                           <li>
                               Item: <?php echo $fetch['product']; ?>
                           </li>
                           <li>
                               Amount: <?php echo $fetch['currency'].$fetch['cost']; ?>
                           </li>
                           <li>
                           ...
                           $select_stores="SELECT * FROM `accounts` WHERE `user_id`=$store_id";
                           $stores_selected=mysqli_query($conn,$select_stores);
                           $stores_fetch=mysqli_fetch_assoc($stores_selected);
                           $business_name=$stores_fetch['business_name'];
                           if(empty($business_name)){
                               $business_name=$stores_fetch['username'];
                           }
                           ?>
                           <li>
                               Store: <?php echo $business_name; ?>
                           </li>
                           <?php 
                           if(empty($fetch['delivery_option'])){
                               $fetch['delivery_option'] = "self pickup";
                           }
                           
                           ?>
                           <li>
                              Delivery Option: <?php echo $fetch['delivery_option']; ?>
                           </li>
                           <?php 
                           if(str_contains($fetch['delivery_option'],"doorstep")){
                              echo '<li>
                               Delivery Fee: '.$fetch['currency'].$fetch['delivery_fee'].'
                           </li>
                           <li>
                               Delivery Address: '.$fetch['delivery_address'].'
                           </li>';
                       ...
                       <hr style="border-style:dashed;border-width:0.1px;background:transparent">
                       <span style="font-size:0.6rem;color:#708090;text-align:center;display:block">
                           <?php
                           if(str_contains($fetch['delivery_option'],"self")){
                               echo 'Kindly mark your order as delivered upon successful pickup. If you do not mark it or contact support, it will automatically be marked as delivered under 3 days. 
                      ';
                           }
                            else{
                                echo 'Kindly mark your order as delivered upon receipt. If techie stores confirm delivery and you do not mark it or contact support, it will automatically be marked as delivered under 3 days. 
                      ';
                            }
