...
.loop_head{
   ...
   .confirm_reset_section *{
    ...
    <style>
        ...
        .checkout_buttons button:nth-of-type(1){
...
Login</a></div></section>';
  ...
  <section class="cart_page_group">
        <section class="section3">
            ...
            <!-- START -->
            <?php 
            $select="SELECT `cart`.*,`accounts`.`business_name` FROM `cart` JOIN `accounts` ON `cart`.`store_id` = `accounts`.`user_id` WHERE `cart`.`uniqid`='$uniqid' ORDER BY `accounts`.`business_name` DESC";
            $selected=mysqli_query($conn,$select);
            if($selected){
                $row=mysqli_num_rows($selected);
                if($row > 0){
                  ...
                  while($fetch=mysqli_fetch_assoc($selected)){
                      ...
                      $select_store="SELECT * FROM `accounts` JOIN `users` ON `users`.`id`=`accounts`.`user_id` WHERE `user_id`=$store_id";
                      $store_selected=mysqli_query($conn,$select_store);
                      $store_fetch=mysqli_fetch_assoc($store_selected);
                      $minimum_order=number_format($store_fetch['minimum_order']);
                      $total=number_format($total);
                      $remaining=number_format($remaining);
                      $currency=$store_fetch['currency'];
        ...
        <section style="display:'.$show_name.';background:silver;color:black" class="loop_group loop_head">
        <strong></strong>
        </section>
        <div style="width:100%;padding:0;display:'.$show_name.'" class="stores '.$class.'">
        <input type="hidden" value="'.$store_id.'" class="'.$show_class.'">
        ...
        <section style="align-items:flex-start" class="loop_group">
            <span class="span_house"><div class="price_group"><span>'.$fetch['currency'].'</span><span class="forever_span">'.number_format($fetch['product_cost'] * $fetch['quantity']).'</span></div>
       ...
       <div class="button_div" style="display:none;">
        ...
        <div style="width:100%;padding:0;display:none" class="stores">
                    <span style="background:#001f3f;color:white;padding:10px">'.$store.'</span> 
                    <section class="stores_det_section">
                        <div class="details_div"><span>minimum order :500ngn</span><span>500ngn of 500ngn</span></div>
                        <div class="stores_progress_parent">
                            <div style="width:75%" class="stores_progress_child">
                                
                            </div>
                        </div>
                        <div style="display:flex;" class="details_div"><span style="font-size:0.7rem;color:red">Add 400ngn more to meet the minimum order amount</span></div>
                        <div style="padding:10px 0">
                            <button onclick="window.location.href=&quot;test&quot;" class="shop_this_store pointer">
                                Shop this Store
                            </button>
                        </div>
                    </section>
    ...
    ';
            ...
            ?>
             ...
             <section style="display:none" class="stores_section">
...
<div class="subtotal_div"><span>Subtotal:</span><span><?php echo $fetch['currency'].'<span class="total">'.number_format($total).'</span>'; ?></span></div>
<div class="subtotal_div"><span>Service Fee:</span><span><?php echo $fetch['currency'].'<span class="service_fee">'.number_format($service_fee).'</span>'; ?></span></div>
...
<hr>
<div class="subtotal_div grand_total"><strong>Grand Total:</strong><strong><?php echo $fetch['currency'].'<span class="all_total">'.number_format($grand).'</span>'; ?></strong></div>
<button class="checkout_button pointer"><span>&#128179;</span>PROCEED TO CHECKOUT</button>
  ...
  <section class="limit_parent">
    ...
    <script src="cart.js?v=6.5"></script> 
   <script>
       function turn(){
